...
KcD offers an interesting way of handling errors related to missing data.
The idea is to throw a custom Response specifying the status code 404.
This can be done in Remix by using something like `throw new Response('Not found', { status: 404 })`.
But in `app/utils/misc.tsx` there is a special handler for this, `invariantResponse`:
