...
The last bit in the error handling is to add a **splat route**. Splat routes
match any route that starts with a given path e.g. `/` and is not matched
by any other route. Also known as "catchall" and "star" segments.
If a route path pattern ends with `/*` then it will match any characters
following the `/`, including other `/` characters.

> It's important to note that splat routes aren't only useful for 404s,
> but they can be really handy for use with a CMS
> where the URL segments are dynamic.

As we're following the `remix-flat-routes` convention, to create a route
that matches `/*`, we'll create a dedicated route file at `app/routes/$.tsx`.

When this route is matched, its `ErrorBoundary` component will be rendered
inside the `outlet` element of the `App` component.

Finally, we'll want to export a default export just in case the loader is changed
or fails in some way, we'll still be able to display some error to the user:
