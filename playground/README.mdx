...
Securitywise it is better to use the same environment variables for both client and server.
For that we use `entry.client.tsx` and `server.client.tsx`.
In the `entry.server.tsx` we have to set the `ENV` variable in the global scope:

```tsx
global.ENV = getEnv()
```

Now, in the `entry.client.tsx` we can access the `ENV` variable from the global scope:
