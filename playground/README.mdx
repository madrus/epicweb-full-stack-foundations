...
I liked the Remix `useNavigation` and `useFormAction` hooks.
They help us synchronize the route and the form action so that we know
...
Now, we can show the "pending UI" using the `isSubmitting` boolean.
...
This will show the spinning loader inside the button as long as the form is submitting.
