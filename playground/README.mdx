...
Resource routes are routes that do not return a default export like components
or pages, but return JSON, or plaintext, or an image.
They can be used for things like health checks, or API calls, e.g. a list of all users.
Basically, they consist of loaders (GET) and/or actions (POST|PUT|DELETE|PATCH).
