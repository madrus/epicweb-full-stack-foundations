...
`DataFunctionArgs` is deprecated but can be just replaced by `LoaderFunctionArgs`.

If we use a loader, we don't need to use `useParams`, as we can get the params
and data from the `useLoaderData`.

An interesting alternative is the way to return data from a loader function.

Option 1: using `Response`:

```tsx
const body = JSON.stringify({user: {name: user.name, username: user.username}})
...
Option 2: using the Remix `json` util:
...
KcD recommends the second one but he advises to learn both.
